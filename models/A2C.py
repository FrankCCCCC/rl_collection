...
class Agent:
    def __init__(self, state_size, num_action, reward_discount, learning_rate, exploration_strategy):
        ...
        self.actor, self.critic = self.build_model('actor', 'critic')
        self.is_shutdown_explore = False

        self.buffer = []
        self.reset_buffer()

    def build_model(self, actor_name, critic_name):
        # Actor Model
        actor_nn_input = tf.keras.Input(shape = self.state_size, dtype = self.data_type)
        ...
        actor_nn_output = tf.keras.activations.softmax(x)

        actor_model = tf.keras.Model(name = actor_name, inputs = actor_nn_input, outputs = actor_nn_output)

        # Critic Model
        critic_nn_input = tf.keras.Input(shape = self.state_size, dtype = self.data_type)
        ...
        x = tf.keras.layers.ReLU()(x)
        x = tf.keras.layers.Dense(units = self.num_action)(x)
        critic_nn_output = tf.keras.activations.softmax(x)

        critic_model = tf.keras.Model(name = critic_name, inputs = critic_nn_input, outputs = critic_nn_output)
